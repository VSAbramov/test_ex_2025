...
1. __Материализация отчета (Materialized View)__: Учитывая, что отчет «Топ-5 товаров» актуален на протяжении месяца и не требует данных в реальном времени, оптимальным решением будет замена VIEW на материализованное представление (Materialized View) или отдельную таблицу-аггрегат. Это позволит выполнять тяжелый запрос всего один раз (например, в начале месяца или ночью), а пользователи будут обращаться к уже готовым предварительно рассчитанным данным.
...
Конечно зависит от ситуации, но скорее всего нет большого смысла оптимизировать 
поиск корневых категорий, их не должно быть настолько много, чтобы заметно 
